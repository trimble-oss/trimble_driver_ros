...
| `estaimte_gsof_time`                    | `bool`         | This will use the nearest PositionTime (#1) or INSSolution (#49) message as the header timestamp for messages that do not have GPS time in the GSOF protocol.  To get the most accurate time stamp, one of these two messages should be broadast equal to the greater than the fastest GSOF update rate. If this is disabled, some GSOF header timestamps will be at 0 sec, 0 ns.                     | 
